...
var recData = [];
var initQuery;
var defaultQuery = "MATCH (a:Article) RETURN *"
var defaultQueryWithExclusion = "MATCH (a:Article) RETURN *"
...
function infoDisplayMessage(msg, type) {
    ...
    if (type == "entity") {
        // //ids
        // infoMessage=addHtmlELement(infoMessage,"Article IDs:","strong");
        ...
        //Article ID
        infoMessage = addHtmlELement(infoMessage, "Article ID:", "strong");
        ...
        if (params.nodes.length == 0) {
            ...
            if (title.includes("http")) { // news news node is clicked
                ...
                if (clickCount % 5 == 0) {
                    ...
                    var recommendationQuery = "MATCH (a:Article)-[r]-(b:Entity),(n:Article)-[x]-(e:Entity),(p:Article)-[t]-(q:Entity) WHERE  a.Date>\"2020-05-19\" and a.Nid=n.Nid and a.Nid=p.Nid  and (a.Date>" + currentEndDate + " OR a.Date<" + currentStartDate + ")  and (n.Date=a.Date) and (p.Date=a.Date)  and b.Name=" + "\"" + firstEntityName + "\"  and ( e.Name=" + "\"" + secondEntityName + "\" AND q.Name=" + "\"" + thirdEntityName + "\" )  " + " RETURN a.Nid";

                    if (secondEntityName == "" && thirdEntityName == "") {
                        recommendationQuery = "MATCH (a:Article)-[r]-(b:Entity),(n:Article)-[x]-(e:Entity) WHERE a.Date>\"2020-05-19\" and  a.Nid=n.Nid  and (a.Date>" + currentEndDate + " OR a.Date<" + currentStartDate + ")  and (n.Date=a.Date)  and b.Name=" + "\"" + firstEntityName + "\" " + " RETURN a.Nid";
                        await recommendationFetching(recommendationQuery);
                    }
                    else if (secondEntityName != "" && thirdEntityName == "") {
                        recommendationQuery = "MATCH (a:Article)-[r]-(b:Entity),(n:Article)-[x]-(e:Entity) WHERE a.Date>\"2020-05-19\" and  a.Nid=n.Nid  and (a.Date>" + currentEndDate + " OR a.Date<" + currentStartDate + ")  and (n.Date=a.Date)  and b.Name=" + "\"" + firstEntityName + "\"  and ( e.Name=" + "\"" + secondEntityName + "\" ) " + " RETURN a.Nid";
                        await recommendationFetching(recommendationQuery);
                    }
                    else {
                        await recommendationFetching(recommendationQuery);
                        if (recData.length == 0) {
                            recommendationQuery = "MATCH (a:Article)-[r]-(b:Entity),(n:Article)-[x]-(e:Entity) WHERE  a.Date>\"2020-05-19\" and a.Nid=n.Nid  and (a.Date>" + currentEndDate + " OR a.Date<" + currentStartDate + ")  and (n.Date=a.Date)  and b.Name=" + "\"" + firstEntityName + "\"  and ( e.Name=" + "\"" + secondEntityName + "\" OR e.Name=" + "\"" + thirdEntityName + "\" )  " + " RETURN a.Nid";
                            await recommendationFetching(recommendationQuery);
                        }
                    }
                    ...
                    if (recData.length == 0) {
                        ...
                        var recommendationQuery = "MATCH (a:Article)-[r]-(b:Entity),(n:Article)-[x]-(e:Entity) WHERE a.Date>\"2020-05-19\" and  a.Nid=n.Nid  and (a.Date>" + currentEndDate + " OR a.Date<" + currentStartDate + ") and (n.Date=a.Date)  and b.Name=" + "\"" + firstEntityName + "\" " + " RETURN a.Nid";
                        await recommendationFetching(recommendationQuery);
...
function fetchAllData(type) {
    if (type == "withRelations") {
        let query;
        query = alterQuery(defaultQuery, "MATCH", "(a:Article)-[r]-(b:Entity)");
        ...
        let query;
        query = alterQuery(defaultQuery, "MATCH", "(a:Article)");
        query = alterQuery(query, "WHERE", "a.Date>=" + currentStartDate);
        query = alterQuery(query, "WHERE", "a.Date<=" + currentEndDate);
        return new Promise(resolve => {
            viz.onlyData(query).then(value => {
...
async function checkboxFunction() {
    ...
    var query = "MATCH (n:Article)-[r]-(e:Entity) WHERE ( e.Type=\"NoType\" ";
...
async function showAlsoCheckboxFunctions() {
    ...
    var query = "MATCH (n:Article) WHERE ( true  ";
...
async function newsCheckboxFunction() {
    ...
    if (!others.checked) {
        query = "MATCH (n:Article) WHERE ( n.Type=\"NoType\" ";
        if (ex0.checked) {
            none = false;
        ...
        query = "MATCH (n:Article) WHERE ( true ";
        if (!ex0.checked) {
            query = query + "AND n.category<>\"son-dakika\" ";
        }
        if (!ex1.checked) {
